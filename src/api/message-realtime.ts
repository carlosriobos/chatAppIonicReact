...
export class MessageRealtime {
    send(
        ...
        try {
            ...
            const messageRef = ref(db, `channel/${channelId}/message/${messageId}`);
            set(messageRef, {
                id:messageId,
                message,
                userId,
    ...
    getAll(
        ...
        try {
            const db = getDatabase();
            const messagesRef = ref(db, `channel/${channelId}/message`);
    ...
    update(channelId: string, message: MessageModel, newMessage: string) {
        try {
            const db = getDatabase();
            const messageRef = ref(db, `channel/${channelId}/message/${message.id}`);
            set(messageRef, {
                ...message,
                message: newMessage,
            })
